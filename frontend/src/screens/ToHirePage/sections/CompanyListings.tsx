                ...
                <CardContent className="p-5 space-y-3">
                  ...
                  <div className={`overflow-hidden transition-all duration-500 ${isOpen ? "max-h-96" : "max-h-0"}`}>
                    <div className="bg-white rounded-2xl px-5 py-4 mt-2 space-y-2 text-sm shadow-inner">
                      <p className="font-semibold">In need of: <span className="text-[#ff9d00]">{listing.in_need_of}</span></p>
                      <p>Time: <span className="font-semibold">{listing.time_in} - {listing.time_out}</span></p>
                      <p>Salary: <span className="font-semibold">₱{listing.salary}</span></p>
                      <p>Progress: <span className="font-semibold">{listing.progress || 0}/{listing.work_days} days</span></p>
          ...
          <div className="bg-[#ff9d00] rounded-[3rem] p-6 shadow-2xl w-full max-w-[650px]">
            ...
            <div className="bg-white rounded-[2.5rem] p-8 space-y-6 shadow-inner">
              <div className="text-center">
                <h3 className="text-xl font-bold text-black mb-2">In need of:</h3>
                <input
                  type="text"
                  placeholder="Job Title (Quantity)"
                  value={formData.in_need_of}
                  onChange={(e) => handleInputChange("in_need_of", e.target.value)}
              ...
              <div className="space-y-4 px-4">
                {[
                  ...
                  { label: "Total Work Days", key: "work_days" },
                  { label: "Location", key: "location" },
                ].map((field) => (
                  <div key={field.key} className="flex items-center gap-4">
                    <span className="min-w-[140px] font-bold text-gray-700">{field.label} :</span>
                    <input
                      type="text"
                      value={formData[field.key as keyof NewListing]}
                      onChange={(e) => handleInputChange(field.key as keyof NewListing, e.target.value)}
