...
export const createListing = (req, res) => {
  ...
  db.query(sql, [employerId, position, inNeedOf, timeIn, timeOut, salary, workDays, location], (err, result) => {
    if (err) {
...
export const getCompanyProfile = (req, res) => {
  ...
  db.query(sql, [userId], (err, rows) => {
    ...
    if (rows.length === 0) {
      return res.status(200).json({ companyName: "", businessType: "", registrationNumber: "", yearEstablished: "", companyAddress: "", contactNumber: "", companyEmail: "" });
    }
...
export const updateCompanyProfile = (req, res) => {
  const userId = req.user.id;
  const { companyName, businessType, registrationNumber, yearEstablished, companyAddress, contactNumber, companyEmail } = req.body;
  ...
  db.query(sql, [userId, companyName, companyEmail, contactNumber, companyAddress, businessType, registrationNumber, yearEstablished], (err) => {
